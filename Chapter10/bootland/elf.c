...
static int elf_load_program(struct elf_header* header)
{
    ...
    for(i = 0; i < header->program_header_num ; i++){
        ...
        if(phdr->type != 1) continue;   /* check loadable segment */

        memcpy((char*)phdr->physical_addr, (char*)(char*)header + phdr->offset, phdr->file_size);       /* load elf_data to physical_addr */
        memset((char*)phdr->physical_addr + phdr->file_size, 0, phdr->memory_size - phdr->file_size);   /* zero clear non load area (for bss section) */
    }
