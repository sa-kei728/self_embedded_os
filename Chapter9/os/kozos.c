...
static kz_thread_id_t thread_run(kz_func_t func, char* name,
                                int priority, int stacksize, int argc, char* argv[])
{
    int i;
    kz_thread*  thp;
    uint32*     sp;
    ...
    *(--sp) = (uint32)thread_end;
     ...
     *  if priority is 0, this thread is set interrupt mask.
     */
...
{
    /* wake up thread return readyque*/
    putcurrent();

    /* id thread connect readyque and wake up*/
...
static void schedule(void)
{
    ...
    /*
     *  look for readyque in order to high priority, find able to do thread.
     */
